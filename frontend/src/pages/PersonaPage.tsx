      ...
      <Section>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm">
            ...
            <Select value={defaultPatch.target_role} onChange={() => {}} aria-label="目标岗位">
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <Select value={defaultPatch.target_city} onChange={() => {}} aria-label="目标城市">
              <option value="">选择目标城市</option>
              {(scopesQuery.data?.cities ?? []).map((c) => (
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <Input value={defaultPatch.current_title} onChange={() => {}} />
          </label>
          <label className="flex flex-col gap-1 text-sm">
            <span className="text-muted">当前城市</span>
            <Input value={defaultPatch.current_city} onChange={() => {}} />
          </label>
          <label className="flex flex-col gap-1 text-sm">
            <span className="text-muted">工作经验</span>
            <Input value={defaultPatch.total_experience} onChange={() => {}} placeholder="如 3 年" />
          </label>
          <label className="flex flex-col gap-1 text-sm">
            <span className="text-muted">学历</span>
            <Input value={defaultPatch.education} onChange={() => {}} placeholder="如 本科" />
          </label>
        </div>
        <div className="mt-4">
          <Button variant="secondary">保存画像（占位）</Button>
        </div>
      </Section>
    </div>
  )
