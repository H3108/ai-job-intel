...
export function Section({ title, desc, children, topSpace, className }: { title?: string; desc?: string; children?: React.ReactNode; topSpace?: boolean; className?: string }) { return (<section className={`${topSpace?"":"pt-2"} ${className||""}`}>{(title||desc)&&(<div className="mb-3">{title&&<h2 className="font-display text-lg font-semibold text-text">{title}</h2>}{desc&&<p className="mt-1 text-sm text-muted">{desc}</p>}</div>)}<div className="rounded-2xl border border-border bg-surface px-4 py-4 shadow">{children}</div></section>) }
